...
function setupWorld()
  ...
  background = flash.new("com.inoah.ro.LuaDisplayObject", rootsprite, "renderBackground")
  
  rttsprite = flash.new("starling.display.Sprite")
  rtttex = flash.new("starling.textures.RenderTexture", _width, _height, true)
  rttimg = flash.new("starling.display.Image", rtttex)
  rttsprite:addChild(background)

  rootsprite:addChild(rttimg)

  fadeimg = flash.new("starling.display.Image", rtttex)
  fadeimg.alpha = 0.9
  local texscale = 0.95
  fadeimg.scaleX = texscale
  fadeimg.scaleY = texscale
  flash.setx(fadeimg, (_width*(1 - texscale)) / 2)
  flash.sety(fadeimg, (_height*(1 - texscale)) / 2)

  fadetex = flash.new("starling.textures.RenderTexture", _width, _height, true)
  fullimg = flash.new("starling.display.Image", fadetex)
...
function starlingUpdate()
  ...
  for i,bunny in ipairs(bunnies) do
    ...
    bunny.img.rotation = flash.getnumber(bunny.img, "rotation") + bunny.speedR
  ...
  rtttex:clear()
  rtttex:draw(fullimg)
  rtttex:draw(rttsprite)
  fadetex:clear()
  fadetex:draw(fadeimg)
